 ...
 * @version 1.0
 */
public class PAMaster extends ProActiveMaster<PATask,String>{
	/**
	 * 
	 */
	private static final long serialVersionUID = 5908018174132785260L;
	private static final PAMaster master=new PAMaster("dis");
	/*try {
	...
	private static Vector<PATask> tasks = new Vector<PATask>();
	...
	private static HashMap<String, PATask> TaskMap=new HashMap<String,PATask>();
	 ...
	 */
	public synchronized static ProActiveMaster<PATask,String> getMaster() {
		return master;
	}
	 ...
	 */
	public boolean setTaskMap(PATask task) {
	...
	public synchronized boolean submitTask(String taskname, String content,
		...
		PATask tasktmp = new PATask(taskname, content, userId, resultFolder);// 构造新任务
		if (master.setTaskMap(tasktmp)) {
			System.out.println("set task map success!");
		}
		return tasks.add(tasktmp);// 将任务加入任务列表
	}
	 ...
	 */
	public PATask getTaskByQueryId(String taskQueryId) {
		if (TaskMap.containsKey(taskQueryId)) {
	 ...
	 * @return TaskMap
	 */
	public HashMap<String, PATask> getTaskMap() {
		return TaskMap;
	}
